...
func (c Config) Add(name string, s Szenario, ut []*UserType) Szenario {
	...
	slog.Debug("Initialising szenario", log.Szenario, s.Name())
	for _, sz := range c.userTypes[c.allSz.Name].Szenarios {
		if sz.Name() == s.Name() {
			slog.Error("Szenario already exists", log.Szenario, s.Name())
			//return sz
		}
	}
	if err := c.addUserType(c.allSz, s); err != nil {
		slog.Error("Cannot add szenario to usertype all: %v", log.Szenario, s.Name(), log.Error, err)
	}
	for _, t := range ut {
		if err := c.addUserType(t, s); err != nil {
			slog.Error("Adding usertype to szenario", log.Szenario, s.Name(), "user_type", t, log.Error, err)
		}
	}
	return s
}
